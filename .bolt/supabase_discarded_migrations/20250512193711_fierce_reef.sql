/*
  # Полная схема базы данных "Ветеринарный ассистент"

  Создает все необходимые таблицы и настраивает политики безопасности
  для системы управления животноводческим хозяйством.
*/
...
BEGIN
  ...
  -- Политика для добавления сотрудников
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'employees' AND policyname = 'Admins and Managers can insert into employees table'
  ) THEN
  ...
  -- Политика для обновления сотрудников
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
  ...
  -- Политика для удаления сотрудников
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
...
BEGIN
  ...
  -- Политика для модификации администраторами, менеджерами и зоотехниками
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
...
BEGIN
  ...
  -- Политика для модификации администраторами, менеджерами и ветеринарами
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
...
BEGIN
  ...
  -- Политика для модификации администраторами, менеджерами и осеменаторами
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'bulls' AND policyname = 'Bulls can be modified by authorized users'
  ) THEN
    CREATE POLICY "Bulls can be modified by authorized users"
      ON public.bulls
      FOR ALL
      TO authenticated
      USING (
