/*
  # Создание триггерных функций и триггеров для автоматизации бизнес-логики
  
  1. Новые функции
    - `sync_animal_group_changes()` - синхронизирует изменения группы животного
    - `sync_animal_status_changes()` - обрабатывает изменения статуса животного
    - `update_animal_on_treatment_completion()` - обновляет статус животного при завершении лечения
    - `update_animal_on_pregnancy_test()` - обновляет статус животного при тесте стельности
  
  2. Триггеры
    - Триггеры для таблицы animals
    - Триггер для таблицы active_treatments
    - Триггер для таблицы pregnancy_tests
*/
