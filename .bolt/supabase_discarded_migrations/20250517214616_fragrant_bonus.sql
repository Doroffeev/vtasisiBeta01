/*
  # Создание таблицы animals и настройка каскадного удаления
  
  1. Новые таблицы
    - `animals` - таблица животных
  ...
  2. Изменения
    - Добавление каскадного удаления для связанных таблиц
    - Добавление триггеров для синхронизации данных
...
BEGIN
  ...
  IF OLD.group_id IS DISTINCT FROM NEW.group_id AND NEW.group_id IS NOT NULL AND OLD.group_id IS NOT NULL THEN
