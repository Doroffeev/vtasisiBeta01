/*
  ...
  1. Изменения:
    - Добавление проверок для числовых полей в таблицах
    - Проверка существования колонок перед добавлением ограничений
    - Безопасное преобразование типов данных
  
  2. Безопасность:
    - Все операции обернуты в проверки существования
    - Предотвращение ошибок при повторном выполнении
*/
...
BEGIN
  ...
  IF EXISTS (
    ...
    -- Проверяем существование колонки weight перед добавлением ограничения
    IF EXISTS (
      SELECT 1 FROM information_schema.columns
      WHERE table_schema = 'public' AND table_name = 'animals' AND column_name = 'weight'
    ) THEN
      -- Добавляем проверку для поля weight (вес)
      IF NOT EXISTS (
        SELECT 1 FROM information_schema.constraint_column_usage
        WHERE table_schema = 'public' AND table_name = 'animals' AND column_name = 'weight' AND constraint_name LIKE '%check%'
      ) THEN
        ALTER TABLE public.animals
        ADD CONSTRAINT animals_weight_check CHECK (weight IS NULL OR weight::numeric >= 0);
      END IF;
    END IF;
    
    -- Проверяем существование колонки lactation перед добавлением ограничения
    IF EXISTS (
      SELECT 1 FROM information_schema.columns
      WHERE table_schema = 'public' AND table_name = 'animals' AND column_name = 'lactation'
    ) THEN
      -- Добавляем проверку для поля lactation (лактация)
      IF NOT EXISTS (
        SELECT 1 FROM information_schema.constraint_column_usage
        WHERE table_schema = 'public' AND table_name = 'animals' AND column_name = 'lactation' AND constraint_name LIKE '%check%'
      ) THEN
        ALTER TABLE public.animals
        ADD CONSTRAINT animals_lactation_check CHECK (lactation IS NULL OR lactation::numeric >= 0);
      END IF;
    END IF;
    
    -- Проверяем существование колонки insemination_count перед добавлением ограничения
    IF EXISTS (
      SELECT 1 FROM information_schema.columns
      WHERE table_schema = 'public' AND table_name = 'animals' AND column_name = 'insemination_count'
    ) THEN
      -- Добавляем проверку для поля insemination_count (количество осеменений)
      IF NOT EXISTS (
        SELECT 1 FROM information_schema.constraint_column_usage
        WHERE table_schema = 'public' AND table_name = 'animals' AND column_name = 'insemination_count' AND constraint_name LIKE '%check%'
      ) THEN
        ALTER TABLE public.animals
...
BEGIN
  ...
  IF EXISTS (
    ...
    IF NOT EXISTS (
      SELECT 1 FROM information_schema.constraint_column_usage
      WHERE table_schema = 'public' AND table_name = 'bulls' AND column_name = 'price' AND constraint_name LIKE '%check%'
    ) THEN
    ...
    IF NOT EXISTS (
      SELECT 1 FROM information_schema.constraint_column_usage
      WHERE table_schema = 'public' AND table_name = 'bulls' AND column_name = 'remaining_doses' AND constraint_name LIKE '%check%'
    ) THEN
      ALTER TABLE public.bulls
...
BEGIN
  ...
  IF EXISTS (
    ...
    IF NOT EXISTS (
      SELECT 1 FROM information_schema.constraint_column_usage
      WHERE table_schema = 'public' AND table_name = 'medications' AND column_name = 'unit_price' AND constraint_name LIKE '%check%'
    ) THEN
